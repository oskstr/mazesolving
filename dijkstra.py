...
def solve(maze):
    ...
    # Distances holds the distance to any node taking
    # the best known path so far. Better paths replace
    # worse ones as we find them.
    ...
    # shorter path is found. This isn't hugely memory efficient,
    # but likely to be faster than a dictionary or similar.
    nodeindex = [None] * total

    # To begin, we set the distance to the start to zero (we're there)
    # and add it into the unvisited queue
    ...
    while len(unvisited) > 0:
        ...
        for v in u.neighbours:
            if v != None:
                ...
                # The extra distance from where we are (upos) to
                # the neighbour (vpos) - this is manhattan distance
    ...
    # We want to reconstruct the path. We start at end, and then go
    # prev[end] and follow all the prev[] links until we're back at the start
